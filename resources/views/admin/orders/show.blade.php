                ...
                <div class="card-body">
                    <table class="table">
                        <thead>
                            ...
                            <th>  طريقة الدفع </th>
                            @if($order->payment_method == "bank_transfer")      <th>   اسم صاحب التحويل </th>


                            <th>   رقم التحويل </th>

                            <th>   ملف التحويل </th>@endif
                            ...
                            <tr>
                               ...
                               <td>{{\App\Http\Controllers\OrderController::get_payment_method($order->id)}}</td>
                                @if($order->payment_method == "bank_transfer")
                                 @php $transfer= DB::selectOne("select * from bank_transfer where order_id = $order->id") @endphp
                                 <td>{{$transfer->name}}</td>

                                 <td>{{$transfer->number}}</td>
                                     <td> <a href="{{URL::to('/uploads/transfers/'.$transfer->file)}}">تحميل</a> </td>
...
<h3>حالات الطلب</h3>
                   ...
                   @if($admin)
                     {{-- <h3>الارسال الي مندوب ؟</h3>
                   <div class="col-md-3" >
                    <form action="{{route('assgined_to',['id'=>$order->id])}}" method="post">
                        @csrf
                        <select class="form-control select2" name="rep_id">
                   ...
                   <h3>الارسال الي عامل طباعة ؟</h3>
                   <div class="col-md-3" >
                    <form action="{{route('assgined_to_printer',['id'=>$order->id])}}" method="post">
                        @csrf
